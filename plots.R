...
# Reading in data
  ...
  setwd("/Users/donalp/Dropbox/3-Work/Training/Software stuff/Coding/R/DataScience/Coursera/Exploratory data analysis/airquality-assignment")
...
options(scipen=5)
with( plot1data, plot(year, year_total, 
                     pch=19, 
...
with( plot2data, plot(year, year_total, 
                      pch=19, 
...
ggplot(plot3data, aes(x=year, y=year_total, fill=type)) + 
  ...
  ylab("Total emissions") + xlab("Year") + facet_grid( . ~ type )
...
ggplot(plot6data, aes(x=year, y=year_total, colour=city) ) + geom_bar(stat="identity", position="dodge")
